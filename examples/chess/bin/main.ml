...
let () =
  ...
  and m_black =
    Material.(
      v ~ambient:0. ~reflectivity:1.
        ~pattern:Pattern.(v (Solid Colour.black))
        ())
  in
  ...
  let models =
    List.mapi
      (fun i p ->
        ...
        let m =
          Material.v ~ambient:0.2 ~reflectivity:0.9 ~transparency:0.9
            ~refractive_index:1.5 ~diffuse:0.1 ~specular:0.5 ~shininess:300.
  ...
  let pawns =
    List.init 8 (fun i ->
        ...
        let shapes =
          X3d.of_file ~material:m "examples/chess/Chess/pawn white.x3d"
        in
  ...
  let t2t =
    Transformation.(
      combine [ scaling (-1.) 1. 1.; rotate_y Float.pi; translation 0. 0. 6. ])
  in
