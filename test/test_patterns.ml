...
let test_solid_pattern_is_constant _ =
  ...
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 1. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 2. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 1.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 2.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0.9 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 1. 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point (-0.1) 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point (-1.) 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point (-1.1) 0. 0.))
...
let test_stripes_alternates_in_x _ =
  ...
  assert_equal w (Pattern.colour_at p (Specialised.point 0. 0. 0.));
  assert_equal w (Pattern.colour_at p (Specialised.point 0.9 0. 0.));
  assert_equal b (Pattern.colour_at p (Specialised.point 1. 0. 0.));
  assert_equal b (Pattern.colour_at p (Specialised.point (-0.1) 0. 0.));
  assert_equal b (Pattern.colour_at p (Specialised.point (-1.) 0. 0.));
  assert_equal w (Pattern.colour_at p (Specialised.point (-1.1) 0. 0.))

let test_strips_with_object_translation _ =
  let pt = Specialised.point 1.5 0. 0. in
  let t = Transformation.scaling 2. 2. 2. in
  let s = Shape.(v ~transform:t Sphere) in
  let p = Pattern.(v (Stripes (Colour.white, Colour.black))) in

  let opt = Specialised.multiply (Shape.inverse_transform s) pt in
...
let test_strips_with_pattern_translation _ =
  ...
  let res = Pattern.colour_at p (Specialised.point 1.5 0. 0.) in
  assert_equal Colour.white res

let test_strips_with_object_and_pattern_translation _ =
  let pt = Specialised.point 2.5 0. 0. in
...
let test_gradient_pattern _ =
  ...
  assert_equal expected (Pattern.colour_at p (Specialised.point 0.25 0. 0.));
  let expected = Colour.v 0.5 0.5 0.5 in
  assert_equal expected (Pattern.colour_at p (Specialised.point 0.5 0. 0.));
  let expected = Colour.v 0.25 0.25 0.25 in
  assert_equal expected (Pattern.colour_at p (Specialised.point 0.75 0. 0.))
...
let test_ring_pattern _ =
  ...
  assert_equal expected (Pattern.colour_at p (Specialised.point 1. 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 1. 0. 1.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0.708 0. 0.708))
...
let test_cubes_pattern_in_y _ =
  ...
  let expected = Colour.white in
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0.9 0.));
  let expected = Colour.black in
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 1. 0.))
...
let test_cubes_pattern_in_z _ =
  ...
  let expected = Colour.white in
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 0.));
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 0.9));
  let expected = Colour.black in
  assert_equal expected (Pattern.colour_at p (Specialised.point 0. 0. 1.))
