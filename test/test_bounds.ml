...
let test_sphere_bounds _ =
  ...
  let expected_min = Tuple.point (-1.) (-1.) (-1.)
  and expected_max = Tuple.point 1. 1. 1. in
  assert_bool "is equal" (Tuple.is_equal expected_min minb);
  assert_bool "is equal" (Tuple.is_equal expected_max maxb)
...
let test_cylinder_bounds _ =
  ...
  in
  let minb, maxb = Shape.bounds s in
  let expected_min = Tuple.point (-1.) (-3.) (-1.)
  and expected_max = Tuple.point 1. 4. 1. in
  assert_bool "is equal" (Tuple.is_equal expected_min minb);
  assert_bool "is equal" (Tuple.is_equal expected_max maxb)
...
let test_plane_bounds _ =
  ...
  let minb, maxb = Shape.bounds s in
  let expected_min = Tuple.point Float.neg_infinity 0. Float.neg_infinity
  and expected_max = Tuple.point Float.infinity 0. Float.infinity in
  assert_equal (Tuple.x expected_min) (Tuple.x minb);
  assert_equal (Tuple.y expected_min) (Tuple.y minb);
  assert_equal (Tuple.z expected_min) (Tuple.z minb);
  assert_equal (Tuple.x expected_max) (Tuple.x maxb);
  assert_equal (Tuple.y expected_max) (Tuple.y maxb);
  assert_equal (Tuple.z expected_max) (Tuple.z maxb)
...
let test_cone_bounds _ =
  ...
  in
  let minb, maxb = Shape.bounds s in
  let expected_min = Tuple.point (-4.) (-3.) (-4.)
  and expected_max = Tuple.point 4. 4. 4. in
  assert_bool "is equal" (Tuple.is_equal expected_min minb);
  assert_bool "is equal" (Tuple.is_equal expected_max maxb)
...
let test_group_bounds_with_transformed_object _ =
  ...
  let s = Shape.(v (Group [ inner ])) in
  let minb, maxb = Shape.bounds s in
  let expected_min = Tuple.point (-2.) (-3.) (-4.)
  and expected_max = Tuple.point 2. 3. 4. in
  assert_bool "is equal" (Tuple.is_equal expected_min minb);
  assert_bool "is equal" (Tuple.is_equal expected_max maxb)
...
let test_transformed_group_bounds_with_untransformed_object _ =
  ...
  let minb, maxb = Shape.bounds s in
  let expected_min = Tuple.point 0. 1. 2.
  and expected_max = Tuple.point 2. 3. 4. in
  assert_bool "is equal" (Tuple.is_equal expected_min minb);
  assert_bool "is equal" (Tuple.is_equal expected_max maxb)
...
let test_transformed_group_bounds_with_transformed_object _ =
  ...
  let minb, maxb = Shape.bounds s in
  let expected_min = Tuple.point (-1.) (-1.) (-1.)
  and expected_max = Tuple.point 3. 5. 7. in
  assert_bool "is equal" (Tuple.is_equal expected_min minb);
  assert_bool "is equal" (Tuple.is_equal expected_max maxb)
...
let test_group_with_multiple_objects _ =
  ...
  let minb, maxb = Shape.bounds s in
  let expected_min = Tuple.point (-5.) (-1.) (-1.)
  and expected_max = Tuple.point 5. 1. 1. in
  assert_bool "is equal" (Tuple.is_equal expected_min minb);
  assert_bool "is equal" (Tuple.is_equal expected_max maxb)
