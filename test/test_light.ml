...
let test_create_light _ =
  let p = Specialised.point 0. 0. 0. and i = Colour.v 1. 1. 1. in
  let res = Light.v p i in
  assert_bool "is equal" (Specialised.is_equal p (Light.position res));
  assert_bool "is equal" (Colour.is_equal i (Light.intensity res))

let test_create_light_with_vector_fails _ =
  let p = Specialised.vector 0. 0. 0. and i = Colour.v 1. 1. 1. in
